...
export default function PaymentPage() {
  ...
  async function doPayment(method: string) {
    ...
    (async () => {
      try {
        const apiBase = (import.meta.env.VITE_API_BASE as string) || "http://localhost:8000/api";
        const payload: any = { recipe_name: recipe?.recipe_name, amount: recipe?.price };
        // include machine id if configured via env var
        try {
          const machineId = (import.meta.env.VITE_MACHINE_ID as string) || null;
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {paymentMethods.map((method) => (
                  <button
                    ...
                    className={`group relative p-4 rounded-xl border-2 transition-all transform hover:scale-105 ${selectedMethod === method.id
